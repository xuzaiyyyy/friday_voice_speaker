												  ...
												  timer_(new HeapTimer()), threadpool_(new ThreadPool(threadNum)), epoller_(new Epoller())
{
    ...
    HttpConn::srcDir = srcDir_.c_str(); // 传给 HttpConn 的还是 const char*

	// 初始化连接池，这是第一个I/O操作
	SqlConnPool::Instance()->Init("localhost", sqlPort, sqlUser, sqlPwd, dbName, connPoolNum); // 连接池单例的初始化
																							   // 初始化事件和初始化socket(监听)
	InitEventMode_(trigMode);																   // 设置listenevent_ connevent
	if (!InitSocket_())																		   // 初始化套接子 包括 socket bind listen
	{
		isClose_ = true;
	}
...
void WebServer::Start()
{
	...
	while (!isClose_)
	{
		// 阶段一：闹钟设置
		// 查看最早过期的定时器还需要多久
		// 无论有无网络数据，主线程也会在这个时间醒来，去清理过期的连接
		if (timeoutMS_ > 0)
		{
...
{
	assert(client);
	ExtentTime_(client);										   // 这里是为了防止用户因为闹钟时间到而断开连接，每当用户有动作后应该重新设置闹钟
	threadpool_->AddTask(bind(&WebServer::OnRead_, this, client)); // 这是一个右值，bind将参数和函数绑定
}
